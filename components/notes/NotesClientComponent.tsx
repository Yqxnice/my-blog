/**
 * 功能：手记客户端组件
 * 目的：处理手记列表的客户端渲染，包括过滤、展开/收起、模态框等功能
 * 作者：Yqxnice
 */
'use client';
...
// ── 主组件 ────────────────────────────────────────────────────────────────────
export default function NotesClientComponent({ posts, onTagClick }: NotesClientComponentProps & { onTagClick?: (tag: string) => void }) {
  ...
  const filtered = useMemo(() => {
    let result = posts;
    if (activeType) {
      result = result.filter(p => p.noteType === activeType);
    }
    if (activeTag) {
        ...
        <div className="max-w-[860px] mx-auto px-5 md:px-8">
          ...
          <div className="space-y-4 mb-10">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                className={`type-chip type-chip-all${activeType === null && activeTag === null ? ' on' : ''}`}
                onClick={() => { setActiveType(null); setActiveTag(null); setExpandedId(null); }}
              >全部</button>
              {(Object.keys(NOTE_TYPES) as NoteType[]).map(k => {
                ...
                return (
                  <button key={k}
                    ...
                    onClick={() => { setActiveType(on ? null : k); setActiveTag(null); setExpandedId(null); }}
                  >
                    {t.emoji} {t.label}
                  </button>
                );
                ...
                {Array.from(new Set(posts.flatMap(post => post.tags))).map(tag => {
                  ...
                  return (
                    <button key={tag}
                      ...
                      style={on ? { background: 'var(--primary)', borderColor: 'var(--primary)', color: 'var(--primary-foreground)' } : {}}
                      onClick={() => {
                        setActiveTag(on ? null : tag);
                        setExpandedId(null);
                        if (onTagClick) {
                          onTagClick(tag);
                        }
